...
export function Navbar({ dict, lang = 'es' }: { dict: Dictionary; lang?: 'es' | 'en' }) {
  ...
  return (
    <>
      ...
      {!isLegalPage && (
        <div
          className={`fixed inset-0 h-[100dvh] w-screen bg-black/60 backdrop-blur-sm transition-opacity duration-300 md:hidden z-[180] ${
          ...
          <nav aria-label={dict.navbar.navigation} className="flex flex-col gap-4">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                href={link.href}
                onClick={closeMenu}
                className="text-lg font-medium text-white/80 hover:text-white transition-colors duration-200 py-2 border-b border-white/5 last:border-0"
              >
                {link.label}
              </Link>
            ))}
