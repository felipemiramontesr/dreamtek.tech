...
try {
  ...
  if (high > 0 || critical > 0) {
    console.error(
      `❌ [OWASP A06 FAIL] Found ${high} high and ${critical} critical vulnerabilities in production dependencies.`,
    );
    process.exit(1);
  }

  console.log(
    `✅ [OWASP A06 PASS] 0 high and 0 critical vulnerabilities in production dependencies (Total prod vulnerabilities: ${total}).`,
  );
  process.exit(0);
} catch (err) {
  if (err.stdout) {
