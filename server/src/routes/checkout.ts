...
import { renderFinalSettlementReceiptEmail } from '../utils/crm.js';
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.metadata?.tenant_type === 'B2B_LEAD') {
        ...
        const paymentRows = await query<any[]>(
          'SELECT * FROM lead_payments WHERE stripe_session_id = ? AND lead_id = ? LIMIT 1',
          [session.id, leadId],
        );
        ...
        if (
          sessionAmount !== leadPayment.amount_cents ||
          sessionCurrency !== leadPayment.currency
        ) {
          console.warn(
            `[SECURITY_ALERT_FAIL_CLOSED] Webhook B2B discrepancia en monto/divisa. Esperado: ${leadPayment.amount_cents} ${leadPayment.currency}, Recibido: ${sessionAmount} ${sessionCurrency}. Sesión: ${session.id}`,
          );
          res.status(400).json({
            status: 'error',
          ...
          await conn.query(
            `UPDATE lead_payments
             SET status = 'PAID',
                 ...
                 updated_at = NOW()
             WHERE id = ?`,
            [paymentIntentId, leadPayment.id],
          );
      ...
      if (session.metadata?.tenant_type === 'B2B_PROJECT_SETTLEMENT') {
        ...
        const paymentRows = await query<any[]>(
          'SELECT * FROM lead_payments WHERE stripe_session_id = ? AND project_id = ? LIMIT 1',
        ...
        if (
          sessionAmount !== settlementPayment.amount_cents ||
          sessionCurrency !== settlementPayment.currency
        ) {
          console.warn(
            `[SECURITY_ALERT_FAIL_CLOSED] Webhook Finiquito discrepancia en monto/divisa. Esperado: ${settlementPayment.amount_cents} ${settlementPayment.currency}, Recibido: ${sessionAmount} ${sessionCurrency}. Sesión: ${session.id}`,
          );
          res.status(400).json({
            status: 'error',
            error: 'Amount Or Currency Mismatch',
            message:
              'Discrepancia de monto o divisa en la sesión de finiquito respecto al registro pactado.',
          });
          return;
        }
          ...
          await conn.query(
            `UPDATE lead_payments
             SET status = 'PAID',
                 ...
                 updated_at = NOW()
             WHERE id = ?`,
            [paymentIntentId, settlementPayment.id],
          );
          ...
          await conn.query(
            `UPDATE client_projects
             SET paid_amount_cents = paid_amount_cents + ?,
                 ...
                 updated_at = NOW()
             WHERE id = ?`,
            [settlementPayment.amount_cents, projectId],
          );

          if (project.lead_id) {
            await conn.query(
              "UPDATE leads SET deposit_status = 'PAID', updated_at = NOW() WHERE id = ?",
              [project.lead_id],
            );
        ...
        console.log(
          `[SECURITY] PROJECT_FINAL_SETTLEMENT_PAID: Project ${projectId} successfully settled for ${settlementPayment.amount_cents} ${settlementPayment.currency}`,
        );
